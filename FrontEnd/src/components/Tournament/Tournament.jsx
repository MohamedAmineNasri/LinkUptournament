  ...
  useEffect(() => {
    const fetchTournament = async () => {
      ...
      setTournament({
        ...
        date_debut: new Date(tournamentData.date_debut).toLocaleDateString(
          "en-US"
        ),
      });
    };
          ...
          <form class="flex-auto pl-6">
            ...
            <div class="flex space-x-2 mb-4 text-sm font-medium">
              <div class="flex space-x-4">
                <button
                  class="px-6 h-12 uppercase font-semibold tracking-wider border-2 border-black bg-custom-green text-white"
                  type="submit"
                  onClick={() => navigate(`/manage/editt/${tournament._id}`)}
                >
                  Edit
                </button>
              </div>
        ...
        <div>
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark mb-4">
            <div className="py-4 px-4 md:px-6 xl:px-7.5 flex justify-between items-center">
              ...
              {(tournament.type === "Knockout" ||
                tournament.type === "Group stage and Knockout") && (
                <button
